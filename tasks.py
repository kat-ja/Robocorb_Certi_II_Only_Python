...
def store_the_receipt_as_a_PDF_file(row):
    ...
    pdf.html_to_pdf(order_html, '%{ROBOT_ROOT}/output/pdf/'+row["Order number"]+'.pdf')  
    return '%{ROBOT_ROOT}/output/pdf/'+row["Order number"]+'.pdf'

def take_a_screenshot_of_the_robot(row):
    browser.wait_until_element_is_visible('id:robot-preview-image', 2)
    browser.set_screenshot_directory('%{ROBOT_ROOT}/output/screenshots')
...
def go_to_order_another_robot():
    ...
    path = '%{ROBOT_ROOT}/output/pdf/another_robot.pdf'
...
def create_a_ZIP_file_of_the_receipts():
    print(os.listdir('%{ROBOT_ROOT}/output/pdf')) # tulostaa kansion sisällön, hidastaa robottia 1 s
    print(os.listdir('%{ROBOT_ROOT}/output/screenshots')) # tulostaa kansion sisällön, hidastaa robottia 1 s
    archive.archive_folder_with_zip('%{ROBOT_ROOT}/output/pdf', 'pdfs.zip', True)
